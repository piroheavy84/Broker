...
class _DebtsPageState extends ConsumerState<DebtsPage> {
  ...
  final istitutoController = TextEditingController();
  final rataController = TextEditingController();
  final residuoController = TextEditingController();
  ...
  Widget build(BuildContext context) {
    final pratica = ref.watch(practiceProvider);
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Situazione Debitoria"),
      ),
      body: ListView(
        ...
        children: [
          CheckboxListTile(
            value: nessunDebito,
            title: const Text("Il cliente non ha debiti"),
            onChanged: (v) {
              setState(() {
                nessunDebito = v!;
              });
          ...
          if (!nessunDebito) ...[
            ...
            DropdownButtonFormField<String>(
              ...
              items: const [
                DropdownMenuItem(
                  value: "Richiedente 1",
                  child: Text("Richiedente 1"),
                ),
                DropdownMenuItem(
                  value: "Richiedente 2",
                  child: Text("Richiedente 2"),
                ),
                DropdownMenuItem(
                  value: "Richiedente 3",
                  child: Text("Richiedente 3"),
                ),
              ],
              onChanged: (v) {
                setState(() {
            ...
            TextField(
              controller: residuoController,
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(
                labelText: "Capitale Residuo (€)",
              ),
            ),
            ...
            ElevatedButton(
              ...
              child: const Text("AGGIUNGI DEBITO"),
            ),
          ],
          ...
          for (int i = 0; i < pratica.debts.length; i++)
            Card(
              child: ListTile(
                leading: const Icon(
                ...
                title: Text(
                  pratica.debts[i].istituto,
                ),
                subtitle: Text(
                  "Rata € ${pratica.debts[i].rata.toStringAsFixed(2)}",
                ),
                trailing: IconButton(
                  icon: const Icon(
                    Icons.delete,
